...
(() => {
  ...
  runBtn.addEventListener("click", async () => {
    ...
    const trimSec = parseFloat($("trimSec").value || "5");
    ...
    try {
      ...
      const { time, copx, copy } = parsed;
      if (time.length < 20) {
        showMsg("データ点が少なすぎます（20点以上を推奨）。", "warn");
        return;
      }

      // Trim by seconds using time column
      const t0 = time[0] + trimSec;
      const t1 = time[time.length - 1] - trimSec;
      const trimmed = trimByTime(time, copx, copy, t0, t1);
      if (trimmed.time.length < 20) {
        showMsg("前後除外後にデータが少なすぎます。除外秒数を減らしてください。", "warn");
        return;
      }
  ...
  function extractColumns(rows) {
    ...
    for (const r of rows) {
      const t = Number(String(r[timeKey]).trim());
      const x = Number(String(r[xKey]).trim());
      const y = Number(String(r[yKey]).trim());
  ...
  function renderMetrics(res, MF, RPF) {
    ...
    detailsEl.textContent =
      `設定: FS=${res.fs}Hz / LPF=${res.cutoff}Hz / 前後除外=${res.trimSec}s / 単位=${res.unit} / Welch(nperseg=${Math.min(1024,res.n)})`;
  }
