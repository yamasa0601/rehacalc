...
function detectHS(env, fs, opts){
  ...
  for(const k of ks){
    ...
    for(let i=1;i<hs.length;i++){
      if(hs[i] - merged[merged.length-1] >= minGap){
        merged.push(hs[i]);
      }
    }
    hs = merged.map(v => v + offset).filter(v => v>=0 && v<env.length);
    ...
    if(best === null || score < best.score){
      best = {k, thr, hs, score, stats:{n_hs:hs.length, med_step_s:medInt, cv_step:cv, bad_ratio:bad}};
    }
  }

  if(best === null) throw new Error("HS推定に失敗しました（信号が弱い/ノイズ多い/閾値設定不適）。minGapや歩数入力を調整してください。");
  return best;
}
...
function parseCSV(text, filename, fsDefault){
  const lines = text.split(/\r?\n/);

  let start = -1;
  for(let i=0;i<Math.min(lines.length, 500);i++){
    if(lines[i].startsWith("No,")){ start = i; break; }
  }
  ...
  if(start >= 0){
    ...
    let idxV = header.findIndex(h => h.includes("電圧") || h.toLowerCase().includes("mv") || h.toLowerCase().includes("voltage"));
    if(idxV < 0) idxV = 1;
...
function getSelectedSignal(parsed, sigSel, timeSel, fs){
  ...
  const isSeconds = maxT > estDur * 0.8;
  if(isSeconds){
    return {t: t0, x};
  }
...
function computePipeline(t, x, fs, params, expectedCount){
  ...
  return {
    t, envPct, envRaw: env,
    hs: hsRes.hs, k: hsRes.k, thr: hsRes.thr, stats: hsRes.stats,
...
async function handleFile(file, sigSel, timeSel, fs){
  ...
  const parsed = parseCSV(txt, file.name, fs);
