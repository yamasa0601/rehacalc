...
(() => {
  ...
  // FFT.js が読み込まれていない場合のフォールバック（FFT/DFT）
  if (typeof FFT === "undefined") {
    class FFTFallback {
      constructor(size) {
        this.size = size;
        this._isPow2 = (size & (size - 1)) === 0 && size > 0;
        if (this._isPow2) {
          this._cos = new Array(size / 2);
          this._sin = new Array(size / 2);
      ...
      createComplexArray() { return new Array(this.size * 2).fill(0); }
      transform(out, input) {
        if (this._isPow2) {
          this._fft(out, input);
        } else {
      ...
      _fft(out, input) {
        const n = this.size;
        for (let i = 0; i < n * 2; i++) out[i] = input[i] || 0;
      ...
      _dft(out, input) {
        ...
        for (let k = 0; k < n; k++) {
          ...
          let im = 0;
          for (let t = 0; t < n; t++) {
            const xr = input[2 * t] || 0;
            const xi = input[2 * t + 1] || 0;
  ...
  runBtn.addEventListener("click", async () => {
    ...
    const unitIn = $("unit").value; // mm or cm
    const unitOut = $("unitOut") ? $("unitOut").value : "cm";
    const unitToCm = (unitIn === "mm") ? 0.1 : 1.0;
    const cmToOut = (unitOut === "mm") ? 10.0 : 1.0;
    const unitToOut = unitToCm * cmToOut;
    ...
    try {
      ...
      const missing = parsed.missing || { missX: 0, missY: 0 };
      ...
      // 欠損補完（センタリング前）：COPx/COPy が空欄の行は保持し、time軸で線形補間します
      if ((missing.missX || 0) > 0 || (missing.missY || 0) > 0) {
        copx = imputeLinearOnTime(time, copx);
        copy = imputeLinearOnTime(time, copy);
        preprocessNotes.push(`欠損補完：COP欠損（COPx=${missing.missX}行, COPy=${missing.missY}行）を線形補間で補完しました。`);
      }

      // サンプル抜け（time間隔の欠損）を検出。PSD/フィルタの前提（等間隔）のため、補完ON推奨
      const dropouts = countDropouts(time, fs);
      if (dropouts > 0) {
        preprocessNotes.push(`サンプル抜け検出：time間隔の欠損 ${dropouts}回（等間隔リサンプルで補完推奨）`);
        if (!resample) preprocessNotes.push("注意：補完OFFだとPSD/フィルタの前提（等間隔）から外れる可能性があります。");
      }
      ...
      // Convert to output unit after filter
      const xOut = fx.map(v => v * unitToOut);
      const yOut = fy.map(v => v * unitToOut);

      // 95% ellipse area
      const cov = cov2(xOut, yOut);
      const ell = ellipseFromCov(cov);
      const ellipseArea = ell.areaCm2;

      // Convex hull area
      const hull = convexHull(xOut, yOut);
      ...
      const RPF = ratiosTotal.HF;

      const res = {
        fs, cutoff, trimSec, unitIn, unitOut,
        autoTrim, resample,
        note,
  ...
  function extractColumns(rows) {
    ...
    for (const r of rows) {
      ...
      if (!Number.isFinite(x)) missX += 1;
      if (!Number.isFinite(y)) missY += 1;
      time.push(t); copx.push(x); copy.push(y);
    }
    if (time.length < 2) return { ok: false, error: "数値として読める行がありません。" };

    return { ok: true, time, copx, copy, keys: { timeKey, xKey, yKey }, missing: { missX, missY } };
  }
  ...
  function imputeLinearOnTime(time, arr) {
    const n = arr.length;
    const out = arr.slice();
    let lastIdx = null;
    for (let i = 0; i < n; i++) {
      if (Number.isFinite(out[i])) {
        if (lastIdx === null) {
          ...
          const t0 = time[lastIdx];
          const t1 = time[i];
          const v0 = out[lastIdx];
          const v1 = out[i];
          if (Number.isFinite(t0) && Number.isFinite(t1) && t1 !== t0) {
            for (let k = lastIdx + 1; k < i; k++) {
              const w = (time[k] - t0) / (t1 - t0);
              out[k] = v0 + w * (v1 - v0);
            }
          } else {
            for (let k = lastIdx + 1; k < i; k++) out[k] = v0;
          }
        }
        lastIdx = i;
      }
    }
    if (lastIdx !== null && lastIdx < n - 1) {
      for (let k = lastIdx + 1; k < n; k++) out[k] = out[lastIdx];
    }
    return out;
  }

  function countDropouts(time, fs) {
    if (!Number.isFinite(fs) || fs <= 0) return 0;
    const dt = 1 / fs;
    let count = 0;
    for (let i = 1; i < time.length; i++) {
      const d = time[i] - time[i-1];
      if (Number.isFinite(d) && d > dt * 1.5) count += 1;
    }
    return count;
  }
  ...
  function renderMetrics(res, MF, RPF) {
    const areaUnit = (res.unitOut === "mm") ? "mm²" : "cm²";
    const axisUnit = res.unitOut;
    const items = [
      { k: "楕円面積 (95%)", v: fmt(res.ellipseAreaCm2, 3), u: areaUnit },
      { k: "凸包面積", v: fmt(res.hullAreaCm2, 3), u: areaUnit },
    ...
    detailsEl.textContent =
      `設定: FS=${res.fs}Hz / LPF=${res.cutoff}Hz / 前後除外=${res.trimSec}s / 入力単位=${res.unitIn} / 出力単位=${axisUnit} / 補完=${res.resample ? "ON" : "OFF"} / Welch(nperseg=${Math.min(1024,res.n)})` + (res.note ? `
${res.note}` : "");
  }
  ...
  async function renderPlots(res, MF, RPF) {
    ...
    const axisUnit = res.unitOut;
    const layoutCop = {
      title: { text: `COP（楕円面積=${fmt(res.ellipseAreaCm2,2)} ${areaUnit} / 凸包面積=${fmt(res.hullAreaCm2,2)} ${areaUnit}）`, font: { size: 14 } },
      xaxis: { title: `ML (${axisUnit})`, zeroline: true, scaleanchor: "y", scaleratio: 1 },
      yaxis: { title: `AP (${axisUnit})`, zeroline: true },
  ...
  function buildMetricsCsv(res, MF, RPF) {
    const areaUnit = (res.unitOut === "mm") ? "mm2" : "cm2";
    const lines = [];
    lines.push("key,value,unit");
    lines.push(`ellipse_area_95,${res.ellipseAreaCm2},${areaUnit}`);
    lines.push(`convex_hull_area,${res.hullAreaCm2},${areaUnit}`);
    ...
    lines.push(`unit_in,${res.unitIn},`);
    lines.push(`unit_out,${res.unitOut},`);
    return lines.join("\n");
  }
...
(() => {
  ...
  runBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      const res = {
        fs, cutoff, trimSec, unitIn, unitOut, unitToOut,
        autoTrim, resample,
        note,
        n: trimmed.time.length,
        duration: trimmed.time[trimmed.time.length - 1] - trimmed.time[0],
