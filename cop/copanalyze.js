...
function normalizeKey(k) {
  ...
  function extractColumns(rows) {
    ...
    let timeKey = pick("time", "t", "timestamp", "sec", "seconds");
    let xKey = pick("copx", "cop_x", "cop-x", "ml", "x");
    let yKey = pick("copy", "copyy", "cop_y", "cop-y", "ap", "y", "cop_y", "cop-y");

    // Fallback: partial match for header variations
    const findByIncludes = (subs) => {
      const normHeaders = headers.map(h => ({ orig: h, norm: normalizeKey(h) }));
      for (const s of subs) {
        const ss = normalizeKey(s);
        const hit = normHeaders.find(o => o.norm.includes(ss));
        if (hit) return hit.orig;
      }
      return null;
    };

    if (!timeKey) timeKey = findByIncludes(["time", "timestamp", "sec", "second"]);
    if (!xKey) xKey = findByIncludes(["copx", "ml", "cop_x"]);
    if (!yKey) yKey = findByIncludes(["copy", "copyy", "ap", "cop_y"]);
    ...
    // Keep rows where time is readable; allow COPx/COPy to be missing (NaN) so we can impute later.
    for (const r of rows) {
      ...
      if (Number.isFinite(x)) copx.push(x);
      else { copx.push(NaN); missX++; }

      if (Number.isFinite(y)) copy.push(y);
      else { copy.push(NaN); missY++; }
    }

    if (time.length < 2) return { ok: false, error: "time列が数値として読める行がありません。" };

    // Need at least two finite points per axis
    const finiteX = copx.filter(Number.isFinite).length;
    const finiteY = copy.filter(Number.isFinite).length;
    if (finiteX < 2 || finiteY < 2) {
      return { ok: false, error: "COPx/COPy の有効データが少なすぎます（2点以上必要）。" };
    }
...
// --- preprocessing
  ...
  function sortByTime(time, x, y) {
    ...
    for (const i of idx) {
      ...
      if (lastT !== null && t === lastT) {
        // If duplicate timestamps exist, prefer finite values (do not overwrite with NaN)
        nt[nt.length-1] = t;
        if (Number.isFinite(x[i])) nx[nx.length-1] = x[i];
        if (Number.isFinite(y[i])) ny[ny.length-1] = y[i];
      } else {
        nt.push(t);
        nx.push(x[i]);
        ny.push(y[i]);
  ...
  // Count sampling dropouts based on time gaps (returns estimated missing sample count)
  function countDropouts(timeSec, fs) {
    const dtExp = 1 / fs;
    if (!Number.isFinite(dtExp) || dtExp <= 0) return 0;
    if (!timeSec || timeSec.length < 3) return 0;
    ...
    for (let i = 1; i < timeSec.length; i++) {
      ...
      if (dt > dtExp * 1.5) {
        const expectedSteps = Math.round(dt / dtExp);
        miss += Math.max(0, expectedSteps - 1);
      }
    }
    return miss;
  }

  // Impute NaN values on a time grid via piecewise linear interpolation (edge fill)
  function imputeLinearOnTime(time, arr) {
    const n = arr.length;
    const out = arr.slice();
    if (!time || time.length !== n) return out;

    const good = [];
    for (let i = 0; i < n; i++) {
      if (Number.isFinite(time[i]) && Number.isFinite(out[i])) good.push(i);
    }
    ...
    for (let g = 0; g < good.length - 1; g++) {
      const i0 = good[g], i1 = good[g + 1];
      const t0 = time[i0], t1 = time[i1];
      const v0 = out[i0], v1 = out[i1];

      if (!Number.isFinite(t0) || !Number.isFinite(t1) || t1 === t0) {
        for (let k = i0 + 1; k < i1; k++) out[k] = v0;
        continue;
      }
